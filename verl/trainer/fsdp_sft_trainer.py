...
class FSDPSFTTrainer:
    ...
    def fit(self):
        ...
        if rank == 0:
            print("\n=== First Sample Loss Mask ===")
            first_sample = self.train_dataset[0]
            input_ids = first_sample['input_ids'][1:].contiguous()
            loss_mask = first_sample['loss_mask'][:-1]
            
            text = self.tokenizer.decode(input_ids[input_ids != self.tokenizer.pad_token_id])
            ...
            for i, (token_id, mask) in enumerate(zip(input_ids, loss_mask)):
                ...
                if mask == 0:
                    ...
                    unmasked_tokens.append(token)
            
            print(f"Full text: {text}")
            print(f"\nMasked (not learned): {''.join(masked_tokens)}")
            print(f"\nUnmasked (learned): {''.join(unmasked_tokens)}")
            print("===================================\n")

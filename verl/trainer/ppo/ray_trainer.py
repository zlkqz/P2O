...
from search_r1.llm_agent.memory_generation import LLMGenerationManager, GenerationConfig
...
def compute_advantage(
    ...
    if adv_estimator == AdvantageEstimator.GAE:
        ...
        if "turn_split_indices" not in data.non_tensor_batch:
            raise ValueError("GRPO_TURN requires 'turn_split_indices' in non_tensor_batch")
        
        advantages, returns = core_algos.compute_turn_level_grpo_advantage(
            token_level_rewards=data.batch["token_level_rewards"],
            response_mask=data.batch["response_mask"],
            index=data.non_tensor_batch["uid"],
            epsilon=1e-6,
            config=config,
            turn_split_indices=data.non_tensor_batch["turn_split_indices"]
        )
        data.batch["advantages"] = advantages
        data.batch["returns"] = returns
    else:
...
class RayPPOTrainer:
    ...
    def fit(self):
        ...
        gen_config = GenerationConfig(
            max_turns=self.config.actor_rollout_ref.rollout.max_turns,
            ...
            write_memory_url=self.config.retriever.write_memory_url
        )

        train_golden_answers = dict()
        for data in self.train_dataset:
            idx = data.get('id')
            target = data["reward_model"]["ground_truth"]["target"]
            if hasattr(target, "tolist"):
                answer = target.tolist()
            else:
                answer = target
            train_golden_answers[idx] = answer

        generation_manager = LLMGenerationManager(
            tokenizer=self.tokenizer,
            actor_rollout_wg=self.actor_rollout_wg,
            config=gen_config,
            golden_answers=train_golden_answers,

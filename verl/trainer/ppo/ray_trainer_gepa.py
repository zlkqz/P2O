...
class RayPPOTrainer:
    ...
    def _init_gepa_cfg(self) -> dict[str, Any]:
        ...
        default_cfg = {
            ...
            "best_template_k": 3,
            "train_batch_size_after_gepa": None,
            "mini_batch_size_after_gepa": None,
            "mini_batch_size_per_gpu_after_gepa": None,
            "validate_before_and_after_gepa": True,
            "evaluate_pass_at_k_before_training": True,
        ...
        self._validate_log_prob_config(default_cfg["replace_prompts_when_log_prob"], default_cfg["fix_importance_ratio"])
        ...
        train_bsz = default_cfg.get("train_batch_size_after_gepa")
        mini_bsz = default_cfg.get("mini_batch_size_after_gepa")
        mini_bsz_per_gpu = default_cfg.get("mini_batch_size_per_gpu_after_gepa")
        if train_bsz is not None:
            if mini_bsz is None and mini_bsz_per_gpu is None:
                raise ValueError(
                    "[GEPA] When train_batch_size_after_gepa is set, exactly one of "
                    "mini_batch_size_after_gepa or mini_batch_size_per_gpu_after_gepa must be set. "
                    f"Got train_batch_size_after_gepa={train_bsz}, "
                    f"mini_batch_size_after_gepa={mini_bsz}, "
                    f"mini_batch_size_per_gpu_after_gepa={mini_bsz_per_gpu}."
                )
            if mini_bsz is not None and mini_bsz_per_gpu is not None:
                raise ValueError(
                    "[GEPA] mini_batch_size_after_gepa and mini_batch_size_per_gpu_after_gepa are mutually exclusive. "
                    f"Got mini_batch_size_after_gepa={mini_bsz}, "
                    f"mini_batch_size_per_gpu_after_gepa={mini_bsz_per_gpu}."
                )
        elif mini_bsz is not None or mini_bsz_per_gpu is not None:
            raise ValueError(
                "[GEPA] When mini_batch_size_after_gepa or mini_batch_size_per_gpu_after_gepa is set, "
                "train_batch_size_after_gepa must also be set. "
                f"Got train_batch_size_after_gepa={train_bsz}, "
                f"mini_batch_size_after_gepa={mini_bsz}, "
                f"mini_batch_size_per_gpu_after_gepa={mini_bsz_per_gpu}."
            )

        self.auto_run_settings = {"light": 6, "medium": 12, "heavy": 18, "heavy_2": 36, "heavy_3": 54, "heavy_4": 72}
    ...
    @staticmethod
    def _validate_log_prob_config(replace_prompts_when_log_prob, fix_importance_ratio):
        if replace_prompts_when_log_prob and fix_importance_ratio:
            raise ValueError(
                "[GEPA] 不能同时指定replace_prompts_when_log_prob和fix_importance_ratio为True"
            )

    @staticmethod
    def _validate_gepa_ratio(value, name: str, hard_set_min_count: int):
        if isinstance(value, int):
            if value < hard_set_min_count//3:
                raise ValueError(
                    f"[GEPA] {name} (int) must be >= hard_set_min_count//3, got {value}. "
                    "Please set trainer.gepa ratios to an integer >= hard_set_min_count//3 or a float <= 0.3."
                )
    ...
    def _resolve_ratio_count(self, value: int | float, total_size: int) -> int:
        if isinstance(value, int):
            return min(value, total_size//3)
        ratio_count = max(self._gepa_cfg["hard_set_min_count"]//3, int(total_size * value))
        return min(ratio_count, total_size//3)
    ...
    def _split_gepa_dataset(
        ...
        Args:
            ...
            train_dev_ratio: If int (>=hard_set_min_count//3), absolute dev count; if float (<=0.3), dev ratio.
    ...
    def _train_on_gepa_dataset(
        self,
        samples: list[DataProto],
        prompt_templates: str | list[str],
        epochs: int = 1,
    ):
        """
        Train the actor/critic on the hard dataset using the optimized prompt template.

        The inner GRPO update logic closely mirrors the main `fit` loop, but is
        restricted to the provided GEPA hard-set samples and uses the (one or
        more) optimized templates produced by GEPA.

        NOTE: This method does NOT perform validation (no val_dataset usage).
        """
        if epochs <= 0 or len(samples) == 0:
            return

        train_bsz_after = self._gepa_cfg.get("train_batch_size_after_gepa")
        mini_bsz_after = self._gepa_cfg.get("mini_batch_size_after_gepa")
        mini_bsz_per_gpu_after = self._gepa_cfg.get("mini_batch_size_per_gpu_after_gepa")

        original_config = self.config
        temp_config = None
        if train_bsz_after is not None and (mini_bsz_after is not None or mini_bsz_per_gpu_after is not None):
            if mini_bsz_after is not None:
                print(
                    f"[GEPA] Using custom batch sizes: train_batch_size={train_bsz_after}, "
                    f"mini_batch_size={mini_bsz_after}"
                )
            else:
                print(
                    f"[GEPA] Using custom batch sizes: train_batch_size={train_bsz_after}, "
                    f"mini_batch_size_per_gpu={mini_bsz_per_gpu_after}"
                )
            temp_config = OmegaConf.create(OmegaConf.to_container(self.config, resolve=True))
            self.config = temp_config

            temp_config.data.train_batch_size = train_bsz_after

            if mini_bsz_after is not None:
                if hasattr(temp_config.actor_rollout_ref.actor, "ppo_mini_batch_size"):
                    temp_config.actor_rollout_ref.actor.ppo_mini_batch_size = mini_bsz_after

                if hasattr(temp_config.actor_rollout_ref.actor, "ppo_micro_batch_size"):
                    if temp_config.actor_rollout_ref.actor.ppo_micro_batch_size is not None:
                        temp_config.actor_rollout_ref.actor.ppo_micro_batch_size = mini_bsz_after

                if hasattr(temp_config.actor_rollout_ref.rollout, "log_prob_micro_batch_size"):
                    if temp_config.actor_rollout_ref.rollout.log_prob_micro_batch_size is not None:
                        temp_config.actor_rollout_ref.rollout.log_prob_micro_batch_size = mini_bsz_after

                if hasattr(temp_config.actor_rollout_ref.ref, "log_prob_micro_batch_size"):
                    if temp_config.actor_rollout_ref.ref.log_prob_micro_batch_size is not None:
                        temp_config.actor_rollout_ref.ref.log_prob_micro_batch_size = mini_bsz_after

                if self.use_critic and hasattr(temp_config.critic, "ppo_mini_batch_size"):
                    temp_config.critic.ppo_mini_batch_size = mini_bsz_after

                if self.use_critic and hasattr(temp_config.critic, "ppo_micro_batch_size"):
                    if temp_config.critic.ppo_micro_batch_size is not None:
                        temp_config.critic.ppo_micro_batch_size = mini_bsz_after
            else:
                if hasattr(temp_config.actor_rollout_ref.actor, "ppo_micro_batch_size_per_gpu"):
                    if temp_config.actor_rollout_ref.actor.ppo_micro_batch_size_per_gpu is not None:
                        temp_config.actor_rollout_ref.actor.ppo_micro_batch_size_per_gpu = mini_bsz_per_gpu_after

                if hasattr(temp_config.actor_rollout_ref.rollout, "log_prob_micro_batch_size_per_gpu"):
                    if temp_config.actor_rollout_ref.rollout.log_prob_micro_batch_size_per_gpu is not None:
                        temp_config.actor_rollout_ref.rollout.log_prob_micro_batch_size_per_gpu = mini_bsz_per_gpu_after

                if hasattr(temp_config.actor_rollout_ref.ref, "log_prob_micro_batch_size_per_gpu"):
                    if temp_config.actor_rollout_ref.ref.log_prob_micro_batch_size_per_gpu is not None:
                        temp_config.actor_rollout_ref.ref.log_prob_micro_batch_size_per_gpu = mini_bsz_per_gpu_after

                if self.use_critic and hasattr(temp_config.critic, "ppo_micro_batch_size_per_gpu"):
                    if temp_config.critic.ppo_micro_batch_size_per_gpu is not None:
                        temp_config.critic.ppo_micro_batch_size_per_gpu = mini_bsz_per_gpu_after
        ...
        print(
            f"[GEPA] Training on hard set for {epochs} epoch(s) with "
            f"{len(templates)} template(s): select_top_k={len(templates) > 1}"
        )

        n_rollouts = self.config.actor_rollout_ref.rollout.n

        template_indices_per_rollout: list[int]
        ...
        def _build_training_batch(gepa_samples: list[DataProto]) -> DataProto:
            """Build a GEPA training batch with per-rollout templates applied."""
            if len(templates) == 1:
                base_batch = self._build_gepa_batch(gepa_samples, templates[0])
                if n_rollouts > 1:
                    base_batch = base_batch.repeat(repeat_times=n_rollouts, interleave=True)
                return base_batch

            per_row_samples: list[DataProto] = []
            for sample in gepa_samples:
                for tpl_idx in template_indices_per_rollout:
                    tpl = templates[tpl_idx]
                    per_row_samples.append(self._apply_prompt_template_to_sample(sample, tpl, truncation="left"))
            return DataProto.concat(per_row_samples)
        ...
        for local_epoch in range(epochs):
            ...
            for start in range(0, len(indices), batch_size):
                ...
                mb_samples = [samples[i] for i in mb_indices]

                timing_raw: dict[str, float] = {}
                metrics: dict[str, Any] = {}

                with marked_timer("gepa_build_batch", timing_raw, color="green"):
                    batch: DataProto = _build_training_batch(mb_samples)
                
                base_batch_size = len(mb_samples)
                uid_list = [str(uuid.uuid4()) for _ in range(base_batch_size)]
                if n_rollouts > 1:
                    uid_list = [uid for uid in uid_list for _ in range(n_rollouts)]
                ...
                with marked_timer("gepa_step", timing_raw):
                    with marked_timer("gepa_gen", timing_raw, color="red"):
                        if not self.async_rollout_mode:
                            gen_batch_output_padded = self.actor_rollout_wg.generate_sequences(gen_batch_padded)
                        else:
                            gen_batch_output_padded = self.async_rollout_manager.generate_sequences(gen_batch_padded)
                    ...
                    reward_extra_infos_dict: dict[str, list] = {}
                    with marked_timer("gepa_reward", timing_raw, color="yellow"):
                        if self.use_rm:
                            reward_tensor_rm = self.rm_wg.compute_rm_score(batch)
                            batch = batch.union(reward_tensor_rm)
                        ...
                        if response_mask is not None:
                            seq_raw_rewards = (reward_tensor * response_mask).sum(dim=-1)
                        else:
                            seq_raw_rewards = reward_tensor.sum(dim=-1)
                        
                        raw_reward_stats = {
                            "gepa_training/reward_raw/max": seq_raw_rewards.max().item(),
                            "gepa_training/reward_raw/mean": seq_raw_rewards.mean().item(),
                            "gepa_training/reward_raw/min": seq_raw_rewards.min().item(),
                            "gepa_training/reward_raw/std": seq_raw_rewards.std().item(),
                            "gepa_training/reward_raw/median": seq_raw_rewards.median().item(),
                        }
                        metrics.update(raw_reward_stats)

                    if self._gepa_cfg.get("replace_prompts_when_log_prob", False):
                        batch = self._rebuild_batch_input(batch)

                    with marked_timer("gepa_old_log_prob", timing_raw, color="blue"):
                        batch.batch["token_level_scores"] = reward_tensor
                        if self.config.algorithm.use_kl_in_reward:
                            if self.use_reference_policy:
                                if not self.ref_in_actor:
                    ...
                    if self.use_critic:
                        with marked_timer("gepa_values", timing_raw, color="cyan"):
                            values = self.critic_wg.compute_values(batch)
                            batch = batch.union(values)

                    with marked_timer("gepa_adv", timing_raw, color="brown"):
                    ...
                    if self.use_critic:
                        with marked_timer("gepa_update_critic", timing_raw, color="pink"):
                            critic_output = self.critic_wg.update_critic(batch)
                        critic_output_metrics = reduce_metrics(critic_output.meta_info["metrics"])
                        metrics.update(critic_output_metrics)

                    if self._gepa_cfg.get("fix_importance_ratio", False):
                        batch = self._rebuild_batch_input(batch)

                    if self.config.trainer.critic_warmup <= self.global_steps:
                        with marked_timer("gepa_update_actor", timing_raw, color="red"):
                ...
                gepa_data_metrics = {f"gepa_training/{k}": v for k, v in data_metrics.items()}
                metrics.update(gepa_data_metrics)
                
                metrics.update(
                    {
                        "gepa_training/global_step": self.global_steps,
                        "gepa_training/local_epoch": local_epoch,
                        "gepa_training/batch_size": len(mb_samples),
                        "gepa_training/num_templates": len(templates),
                    }
                )
                
                if timing_raw:
                    for timing_key, timing_value in timing_raw.items():
                        metrics[f"gepa_training/timing/{timing_key}"] = timing_value
            ...
            print(
                f"[GEPA] Completed hard-set GRPO epoch {local_epoch + 1}/{epochs} "
                f"on {len(samples)} samples."
            )

        if temp_config is not None:
            self.config = original_config
            print("[GEPA] Restored original configuration (batch sizes unchanged for subsequent GRPO training).")
    ...
    def _finalize_epoch_prompt_optimization(self, epoch_idx: int):
        ...
        validate_enabled = self._gepa_cfg.get("validate_before_and_after_gepa", True)
        if validate_enabled and self.val_reward_fn is not None and self._logger is not None:
            print(f"[GEPA] Epoch {epoch_idx}: Evaluating before GEPA optimization...")
            val_metrics_before = self._validate()
            if val_metrics_before:
                val_metrics_before_prefixed = {
                    f"gepa-eval-before/{k}": v for k, v in val_metrics_before.items()
                }
                val_metrics_before_prefixed["training/global_step"] = self.global_steps
                val_metrics_before_prefixed["training/epoch"] = epoch_idx
                self._logger.log(data=val_metrics_before_prefixed, step=self.global_steps)
                print(f"[GEPA] Epoch {epoch_idx}: Pre-GEPA evaluation completed and logged.")
            self._gepa_resume_info = {"epoch_idx": epoch_idx, "stage": "after_pre_validation"}
            self._save_checkpoint()
            print(f"[GEPA] Saved checkpoint before gepa_with_grpo in epoch {epoch_idx} step {self.global_steps}")
        
        gepa_ran = self._run_gepa_prompt_optimization(epoch_idx=epoch_idx)
        
        if validate_enabled and gepa_ran and self.val_reward_fn is not None and self._logger is not None:
            print(f"[GEPA] Epoch {epoch_idx}: Evaluating after GEPA optimization (before GRPO training)...")
            val_metrics_after = self._validate()
            if val_metrics_after:
                val_metrics_after_prefixed = {
                    f"gepa-eval-after/{k}": v for k, v in val_metrics_after.items()
                }
                val_metrics_after_prefixed["training/global_step"] = self.global_steps
                val_metrics_after_prefixed["training/epoch"] = epoch_idx
                self._logger.log(data=val_metrics_after_prefixed, step=self.global_steps)
                print(f"[GEPA] Epoch {epoch_idx}: Post-GEPA evaluation completed and logged.")
            self._gepa_resume_info = None
            self._save_checkpoint()
            print(f"[GEPA] Saved checkpoint after gepa_with_grpo in epoch {epoch_idx} step {self.global_steps}")
    ...
    def _run_gepa_prompt_optimization(self, epoch_idx: int) -> bool:
        ...
        train_ratio = self._gepa_cfg.get("train_ratio", self._gepa_cfg["hard_set_min_count"]//3)
        dev_ratio = self._gepa_cfg.get("dev_ratio", self._gepa_cfg["hard_set_min_count"]//3)
        ...
        print(
            ...
            f"leftover={len(leftover_samples)}"
        )

        reflection_minibatch = self._gepa_cfg.get("reflection_minibatch_size", 3)
        epochs_after_gepa = max(1, int(self._gepa_cfg.get("epochs_after_gepa", 1)))
        ...
        if self._gepa_cfg.get("evaluate_pass_at_k_before_training", True):
            ...
            print(f"[GEPA] Skipping pass@k evaluation before training (evaluate_pass_at_k_before_training=False)")

        self._train_on_gepa_dataset(grpo_samples, training_templates, epochs=epochs_after_gepa)
        return True
    ...
    def _save_checkpoint(self):
        ...
        gepa_state = {
            ...
            "gepa_resume_info": self._gepa_resume_info,
        }
    ...
    def _load_checkpoint(self):
        ...
        if os.path.exists(gepa_state_path):
            ...
            print(f"Loaded GEPA state: template={self._gepa_epoch_template}, "
    ...
    def _resume_pending_gepa_if_needed(self):
        ...
        if stage != "after_pre_validation":
            print(f"[GEPA] Unknown resume stage {stage}; skipping automatic resume.")
            return
        ...
        validate_enabled = self._gepa_cfg.get("validate_before_and_after_gepa", True)
        if validate_enabled and gepa_ran and self.val_reward_fn is not None and self._logger is not None:
            print(f"[GEPA] Epoch {epoch_idx}: Evaluating after GEPA optimization (resume path)...")
            val_metrics_after = self._validate()
            if val_metrics_after:
                val_metrics_after_prefixed = {
                    f"gepa-eval-after/{k}": v for k, v in val_metrics_after.items()
                ...
                val_metrics_after_prefixed["training/epoch"] = epoch_idx
                self._logger.log(data=val_metrics_after_prefixed, step=self.global_steps)
                print(f"[GEPA] Epoch {epoch_idx}: Post-GEPA evaluation (resume path) completed and logged.")
    ...
    def fit(self):
        ...
        for epoch in range(self.config.trainer.total_epochs):
            if self.is_gepa_loading_from_checkpoint:
                print(f"Skipping GEPA state reset for epoch {epoch}, because of loading from checkpoint.")
            else:

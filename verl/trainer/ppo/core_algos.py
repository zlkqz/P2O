...
def compute_turn_level_grpo_advantage(
    ...
    **kwargs
    ...
    """
    计算Turn-level的GRPO advantage，输入是完整序列，根据split_indices划分turn
    
    Args:
        token_level_rewards: (bs, sequence_length) - 完整序列的token-level rewards
        response_mask: (bs, sequence_length) - 有效位置mask (标准的response_mask)
        index: (bs,) - 每个序列的prompt索引
        epsilon: float - 防止除零的小常数
        config: Optional[AlgoConfig] - 算法配置
        **kwargs: 其他参数，可能包含 turn_split_indices
    
    Returns:
        advantages: (bs, sequence_length) - 整个序列的advantages
        returns: (bs, sequence_length) - returns (与advantages相同)
    """
    if 'turn_split_indices' not in kwargs:
        raise ValueError("GRPO_TURN requires 'turn_split_indices' in kwargs")
    
    turn_split_indices = kwargs['turn_split_indices']
    
    eos_mask = response_mask
    
    batch_size, seq_length = token_level_rewards.shape

    print(f"\n输入信息:")
    print(f"token_level_rewards shape: {token_level_rewards.shape}")
    print(f"eos_mask shape: {eos_mask.shape}")
    print(f"turn_split_indices shape: {turn_split_indices.shape}")
    ...
    turn1_scores = torch.zeros(batch_size, device=token_level_rewards.device)
    turn2_scores = torch.zeros(batch_size, device=token_level_rewards.device)
    
    for i in range(batch_size):
        split_idx = turn_split_indices[i].item()
        
        turn1_rewards = token_level_rewards[i, :split_idx+1]
        turn1_mask_seq = eos_mask[i, :split_idx+1]
        turn1_non_zero = (turn1_rewards != 0) * turn1_mask_seq
        turn1_scores[i] = (turn1_rewards * turn1_non_zero).sum()
        
        if split_idx + 1 < seq_length:
            turn2_rewards = token_level_rewards[i, split_idx+1:]
            turn2_mask_seq = eos_mask[i, split_idx+1:]
            turn2_non_zero = (turn2_rewards != 0) * turn2_mask_seq
            turn2_scores[i] = (turn2_rewards * turn2_non_zero).sum()
    
    id2turn1_scores = defaultdict(list)
    id2turn2_scores = defaultdict(list)
    
    with torch.no_grad():
        for i in range(batch_size):
            prompt_idx = index[i]
            id2turn1_scores[prompt_idx].append(turn1_scores[i])
            id2turn2_scores[prompt_idx].append(turn2_scores[i])
        
        advantages = torch.zeros_like(token_level_rewards)
        
        for prompt_idx in id2turn1_scores:
            turn1_list = id2turn1_scores[prompt_idx]
            if len(turn1_list) > 1:
                turn1_mean = torch.mean(torch.stack(turn1_list))
                turn1_std = torch.std(torch.stack(turn1_list))
            else:
                turn1_mean = torch.tensor(0.0, device=token_level_rewards.device)
                turn1_std = torch.tensor(1.0, device=token_level_rewards.device)
            
            turn2_list = id2turn2_scores[prompt_idx]
            if len(turn2_list) > 1:
                turn2_mean = torch.mean(torch.stack(turn2_list))
                turn2_std = torch.std(torch.stack(turn2_list))
            else:
                turn2_mean = torch.tensor(0.0, device=token_level_rewards.device)
                turn2_std = torch.tensor(1.0, device=token_level_rewards.device)
            
            for i in range(batch_size):
                if index[i] == prompt_idx:
                    split_idx = turn_split_indices[i].item()
                    
                    turn1_advantage = (turn1_scores[i] - turn1_mean) / (turn1_std + epsilon)
                    print(f"Prompt {prompt_idx}, Sequence {i}: Turn 1 advantage={turn1_advantage:.6f}")
                    advantages[i, :split_idx+1] = turn1_advantage * eos_mask[i, :split_idx+1]
                    
                    if split_idx + 1 < seq_length:
                        turn2_advantage = (turn2_scores[i] - turn2_mean) / (turn2_std + epsilon)
                        advantages[i, split_idx+1:] = turn2_advantage * eos_mask[i, split_idx+1:]
                        print(f"Prompt {prompt_idx}, Sequence {i}: Turn 2 advantage={turn2_advantage:.6f}")
...
def compute_policy_loss(
    ...
    Args:
        ...
        template_ratio_mask (torch.Tensor, optional):
            Additional mask for template ratio weighting, shape (batch_size, response_length).
            Applied to pg_losses before aggregation. Defaults to None.
    """
    ...
    if template_ratio_mask is not None:
        pg_losses = pg_losses * template_ratio_mask

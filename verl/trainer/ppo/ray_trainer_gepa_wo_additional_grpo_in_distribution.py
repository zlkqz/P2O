...
class RayPPOTrainer:
    ...
    def __init__(
        ...
        self._gepa_epoch_sample_template_lists: dict[str, list[str]] = {}
        self._gepa_epoch_sample_solved_status: dict[str, bool] = {}
    ...
    def _split_gepa_dataset(
        ...
        """
        Shuffle and split GEPA samples into train/dev subsets (no leftover set).

        Args:
            samples: List of DataProto samples to split.
            dev_ratio: If int (>=hard_set_min_count//2), absolute dev count; if float (<=0.5), dev ratio.
        ...
        if isinstance(dev_ratio, int):
            try:
                if total_size < int(dev_ratio * 1.5):
                    indices = list(range(len(samples)))
                    random.shuffle(indices)
                    mid = len(indices) // 2
                    dev_indices = set(indices[:mid])
                    dev = [samples[i] for i in dev_indices]
                    train = [samples[i] for i in indices if i not in dev_indices]
                    return train, dev
            except Exception as exc:
                print(f"[GEPA] Warning: small-set dev/train split fallback failed ({exc}); falling back to ratio-based split.")
        indices = list(range(len(samples)))
        random.shuffle(indices)
        ...
        train_set = [samples[i] for i in indices if i not in dev_idxs]

        return train_set, dev
    ...
    def _finalize_epoch_prompt_optimization(self, epoch_idx: int):
        ...
        if hard_size < self._gepa_cfg["hard_set_min_count"]:
            ...
            self._gepa_epoch_training_template_dev_scores = {}
            self._gepa_epoch_sample_template_lists = {}
            self._gepa_epoch_sample_solved_status = {}
            return
    ...
    def _run_gepa_prompt_optimization(self, epoch_idx: int) -> bool:
        ...
        train_samples, dev_samples = self._split_gepa_dataset(
            hard_samples,
            dev_ratio=dev_ratio,
        ...
        num_dev = len(dev_samples)
        if num_candidates == 0 or num_dev == 0:
            return False

        best_scores = []
        for sample_idx in range(num_dev):
            sample_scores = []
            for cand_idx in range(num_candidates):
                try:
                    sample_scores.append(val_subscores[cand_idx][sample_idx])
                except Exception:
                    sample_scores.append(0.0)
            best_scores.append(max(sample_scores))
        correct_count = sum(1 for s in best_scores if s == 1.0)
        accuracy = correct_count / num_dev if num_dev > 0 else 0.0
        print(
            f"[GEPA] Dev set size={num_dev}, solved={correct_count}, "
        ...
        for cand_idx in range(num_candidates):
            ...
            for sample_idx in range(num_dev):
                try:
                    if val_subscores[cand_idx][sample_idx] == 1.0:
                        solved.add(sample_idx)
                except Exception:
                    pass
            candidate_covers.append(solved)
        ...
        while True:
            ...
            for cand_idx in range(num_candidates):
                if cand_idx in selected_indices:
                    continue
                new_cov = candidate_covers[cand_idx] - covered_samples
                if len(new_cov) > len(best_new_coverage):
                    best_cand = cand_idx
                    best_new_coverage = new_cov
            if len(best_new_coverage) == 0:
                break
            selected_indices.append(best_cand)
            covered_samples.update(best_new_coverage)
            print(
                f"[GEPA] Greedy select cand {best_cand}: "
                f"new_cover={len(best_new_coverage)}, "
                f"covered={len(covered_samples)}/{num_dev}"
            )

        if not selected_indices:
            raise RuntimeError("[GEPA] Dev上的贪婪帕累托集合为空")

        best_idx = max(selected_indices, key=lambda i: val_aggregate_scores[i])
        best_template = candidates[best_idx]
        best_score = val_aggregate_scores[best_idx]

        self._gepa_epoch_training_templates = [candidates[i] for i in selected_indices]
        self._gepa_epoch_training_template_dev_scores = {
            candidates[i]: val_aggregate_scores[i] for i in selected_indices
        }
        self._gepa_epoch_template = best_template

        print(
            f"[GEPA] Epoch {epoch_idx} greedy Pareto template set size={len(selected_indices)}, "
        ...
        if self._logger is not None:
            gepa_final_metrics = {
                ...
                f"gepa/epoch_{epoch_idx}/final/greedy_pareto_set_size": len(selected_indices),
            }
            if timing_raw:
        ...
        n_rollouts_for_next_epoch = self.config.actor_rollout_ref.rollout.n
        self._gepa_epoch_sample_template_lists = {}
        self._gepa_epoch_sample_solved_status = {}

        def _signature_from_dataproto(sample: DataProto) -> str | None:
            assert sample.non_tensor_batch is not None and "raw_prompt"  in sample.non_tensor_batch
            raw_prompt_arr = sample.non_tensor_batch["raw_prompt"]
            assert raw_prompt_arr is not None and len(raw_prompt_arr) != 0
            messages = list(deepcopy(raw_prompt_arr[0]))
            assert isinstance(messages, list)
            return self._gepa_prompt_signature_from_messages(messages)
                
        dev_sig_to_index: dict[str, int] = {}
        for j, sample in enumerate(dev_samples):
            sig = _signature_from_dataproto(sample)
            if sig is not None:
                dev_sig_to_index[sig] = j
        ...
        def _weights_for_indices(idxs: list[int]) -> list[float]:
            ws = []
            for i in idxs:
                w = float(val_aggregate_scores[i])
                if w <= 0.0:
                    w = 1e-6
                ws.append(w)
            return ws

        sample_same_template = self._gepa_cfg.get("same_template_in_sample", False)

        for sig, dev_idx in dev_sig_to_index.items():
            solving_indices: list[int] = []
            for cand_idx in selected_indices:
                if dev_idx in candidate_covers[cand_idx]:
                    solving_indices.append(cand_idx)
            if not solving_indices:
                solving_indices = list(selected_indices)

            weights = _weights_for_indices(solving_indices)
            if sample_same_template:
                chosen_idx = rng.choices(solving_indices, weights=weights, k=1)[0]
                templates_for_sample = [candidates[chosen_idx]] * n_rollouts_for_next_epoch
            else:
                chosen = rng.choices(solving_indices, weights=weights, k=n_rollouts_for_next_epoch)
                templates_for_sample = [candidates[i] for i in chosen]

            if self._gepa_cfg.get("left_1_no_template_rollout", False):
                templates_for_sample[-1] = ""

            assert len(templates_for_sample) == self.config.actor_rollout_ref.rollout.n
            self._gepa_epoch_sample_template_lists[sig] = templates_for_sample

            self._gepa_epoch_sample_solved_status[sig] = dev_idx in covered_samples

        train_weights = _weights_for_indices(list(selected_indices))
        for sig in train_signatures:
            if sample_same_template:
                chosen_idx = rng.choices(list(selected_indices), weights=train_weights, k=1)[0]
                templates_for_sample = [candidates[chosen_idx]] * n_rollouts_for_next_epoch
            else:
                chosen = rng.choices(list(selected_indices), weights=train_weights, k=n_rollouts_for_next_epoch)
                templates_for_sample = [candidates[i] for i in chosen]
            ...
            self._gepa_epoch_sample_solved_status[sig] = False
        ...
        solved_count = sum(1 for solved in self._gepa_epoch_sample_solved_status.values() if solved)
        unsolved_count = len(self._gepa_epoch_sample_solved_status) - solved_count
        print(
            f"[GEPA] Hard samples status: solved={solved_count}, "
            f"unsolved={unsolved_count}, total={len(self._gepa_epoch_sample_solved_status)}"
        )
    ...
    def _save_checkpoint(self):
        ...
        gepa_state = {
            ...
            "gepa_epoch_sample_template_lists": getattr(
                self, "_gepa_epoch_sample_template_lists", {}
            ),
            "gepa_epoch_sample_solved_status": getattr(
                self, "_gepa_epoch_sample_solved_status", {}
            ),
    ...
    def _load_checkpoint(self):
        ...
        if os.path.exists(gepa_state_path):
            ...
            self._gepa_epoch_sample_template_lists = gepa_state.get("gepa_epoch_sample_template_lists", {})
            self._gepa_epoch_sample_solved_status = gepa_state.get("gepa_epoch_sample_solved_status", {})
            ...
            print(
                ...
                f"_gepa_epoch_sample_template_lists={len(self._gepa_epoch_sample_template_lists)}, "
                f"_gepa_epoch_sample_solved_status={len(self._gepa_epoch_sample_solved_status)} (loaded but unused)"
            )
        else:
            ...
            for batch_dict in self.train_dataloader:
                ...
                if self._gepa_enabled() and self._prev_epoch_hard_signatures:
                    ...
                    if isinstance(raw_prompt_arr, np.ndarray):
                        base_raw_prompt_arr = raw_prompt_arr
                ...
                if self._gepa_enabled() and base_hard_mask is not None and n_rollouts > 0:
                    if not getattr(self, "_gepa_epoch_sample_template_lists", None):
                        raise RuntimeError("self._gepa_epoch_sample_template_lists is None")
                    else:
                        ...
                        if "raw_prompt" in gen_batch.non_tensor_batch:
                            ...
                            raise RuntimeError(f"No 'raw_prompt' in gen_batch.non_tensor_batch")

                        base_signatures: list[str | None] = [None] * base_batch_size
                        base_templates_per_sample: list[list[str] | None] = [None] * base_batch_size
                        if isinstance(base_raw_prompt_arr, np.ndarray):
                            for i in range(base_batch_size):
                                if not base_hard_mask[i]:
                                    continue
                                messages = list(deepcopy(base_raw_prompt_arr[i]))
                                sig = self._gepa_prompt_signature_from_messages(messages)
                                base_signatures[i] = sig
                                tpl_list = self._gepa_epoch_sample_template_lists.get(sig)
                                base_templates_per_sample[i] = tpl_list
                        else:
                            raise RuntimeError("base_hard_mask is not None but base_raw_prompt_arr is None")
                        ...
                        for row_idx in range(total_rows):
                            ...
                            if base_idx >= base_batch_size or not base_hard_mask[base_idx]:
                                continue
                            tpl_list = base_templates_per_sample[base_idx]
                            if not tpl_list:
                                continue
                            rollout_idx = row_idx % n_rollouts
                            assert len(tpl_list) == n_rollouts
                            tpl = tpl_list[rollout_idx]
